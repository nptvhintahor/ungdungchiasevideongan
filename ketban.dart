...
class _KetBanScreenState extends State<KetBanScreen> with SingleTickerProviderStateMixin {
  late TabController _tabController;
  final TextEditingController _searchController = TextEditingController();
  Timer? _debounce; // Kỹ thuật chống spam API khi gõ phím
  ...
  void _onSearch(String value) {
    ...
    // Thiết lập timer mới: Chờ 500ms sau khi ngừng gõ mới gọi API
    _debounce = Timer(const Duration(milliseconds: 500), () async {
      try {
        final users = await ApiService.searchUsers(value, widget.currentUser['email']);
        if (mounted) {
          setState(() {
            searchResults = users;
            isSearching = false;
  ...
  Future<void> _sendRequest(String targetEmail) async {
    try {
      await ApiService.sendFriendRequest(widget.currentUser['email'], targetEmail);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
const SnackBar(content: Text("✅ Đã gửi lời mời kết bạn!")),
        );
        // Cập nhật lại UI tìm kiếm để ẩn nút kết bạn
  ...
  Future<void> _acceptRequest(String requesterEmail) async {
    try {
      await ApiService.acceptFriendRequest(widget.currentUser['email'], requesterEmail);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("🎉 Đã trở thành bạn bè!")),
        );
        _loadAllData(); // Tải lại danh sách bạn bè và lời mời
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.black,
        elevation: 0,
        title: const Text("Kết nối", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            Tab(text: friendRequests.isNotEmpty ? "Lời mời (${friendRequests.length})" : "Lời mời"),
          ],
        ),
      ),
      body: isLoading 
...
child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white54),
        ...
        Expanded(
          child: searchResults.isEmpty && _searchController.text.isNotEmpty && !isSearching
              ? _buildEmptyState("Không tìm thấy người dùng", Icons.search_off)
              : searchResults.isEmpty
                  ? _buildEmptyState("Nhập để tìm bạn mới", Icons.person_search)
  ...
  Widget _buildButton(Map<String, dynamic> user, String type) {
    if (type == "search") {
      ...
      if (user['email'] == widget.currentUser['email']) return const SizedBox();
      
      // Nếu API trả về trạng thái quan hệ (ví dụ: 'friends', 'pending', 'none')
